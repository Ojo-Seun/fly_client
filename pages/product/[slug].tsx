...
function ProductScreen({ product }: productType) {
  ...
  const handleClick = () => {
    const item = updatedItem.qty > 0? updatedItem : { ...product, qty: qtyValue }
          ...
          <div className={styles.grid}>
            <div className={styles.item1}>
              ...
              <div className={styles.smallImageContainer}>
                <div className={styles.smallImages}>
                  {
                    product.image.map((x, index) =><div key={index} className={styles.img}><Image className={x} layout="responsive" onMouseOver={(e)=>handleMouseOver(e)} src={x} alt={product.name} width={100} height={100} /></div>)
                  }
                </div>
              </div>
            </div>
            ...
            <div className={styles.item2}>
              ...
              <h3 className={styles.productName}>{product.name}</h3>
                <div className={styles.rating}><RatingIcons value={product.rating} /><h3>{product.rating}</h3></div>
              </div>
              <h3>${product.price}</h3>
              <div>
                
                <div className={styles.qtyBtn}>
                  <button type='button' onClick={(e)=>changeQuantity('-')} disabled={qtyValue === 1}>-</button>
                  <span>{qtyValue}</span>
                  <button type='button' onClick={(e)=>changeQuantity('+')} disabled={qtyValue === product.countInStock}>+</button>
                </div>
              </div>
            ...
            <div className={styles.item3}>
              ...
              <div className={styles.subTotal}><strong>Sub-Total</strong><span>${product.price * qtyValue}</span></div>
              <div className={styles.shipping}><strong>Shipping</strong><span>${(product.price / 90).toFixed()}</span></div>
                
              <hr style={{border:"1px solid black"}}/>
              <div className={styles.productTotal}><strong>Total</strong><span>${(product.price * qtyValue + (product.price/90)).toFixed()}</span></div>
              </div>

              <div style={{ textAlign: "start" }}>
                {
                  qtyValue < product.countInStock? <strong style={{ color: "green" }}>In Stock</strong> :<strong style={{ color: "red" }}>Out Of Stock</strong>
                }
            ...
            <div className={styles.item4}>
              ...
              <ul className={styles.specs}>
                ...
                <li>Dimensions: {' '} {product.dimention.height} X {product.dimention.width}</li>
