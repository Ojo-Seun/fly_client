...
const {state, dispatch}= useContext(Store)
  const { loginInfo, cart: { cartItems, shippingAddress, paymentMethod } } = state
  ...
  return (
      <Layout title='Place Order' description='Place Order Page'>
        ...
        <div style={{textAlign:"center"}}>{status.error && <span style={{ color: "red" }}>{status.error}</span>}</div>
            <div className={styles.grid}>
                    <div className={styles.shoppingCart}>
                      
                      <div className={styles.shippingAddress}><strong>Shipping Address</strong><p>{`${shippingAddress.address} ${shippingAddress.country}` }</p></div>
                      <div className={styles.paymentMethod}><strong>Payment Method</strong><p>{paymentMethod}</p></div>
